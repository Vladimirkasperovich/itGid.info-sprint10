...
let ar1 = [1, "Vova", true]; // переменную обьявляем здесь!!!!
...
let ar6 = ["Vladimir", "Taurus", 14, "May"];

function f6() {
  document.querySelector(
    ".out-6"
  ).textContent = `${ar6[0]} ${ar6[1]} ${ar6[2]} ${ar6[3]}`;
}
...
function f10() {
  document.querySelector(".out-10").textContent =
    ar10[ar10.length - 6] + ar10[ar10.length - 1];
}
...
function f16() {
  ...
  for (let i = 0; i < ar16.length; i++) {
    if (ar16[i] % 2 === 0) ar16_even.push(ar16[i]);
  ...
  document.querySelector(".out-16-even").textContent = even;
  document.querySelector(".out-16-odd").textContent = odd;
...
function f17() {
    ...
    document.querySelector('.out-17').textContent = sum;
}
